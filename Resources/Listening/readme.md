...
| [`Kansai-ben Primer (kansaibenkyou.net, CC BY-SA 3.0)/Extracted Data (JSON)/`](<./Kansai-ben Primer (kansaibenkyou.net, CC BY-SA 3.0)/Extracted Data (JSON)>)                                                                                                                                | 4 `.json` files, 364 KB                               | The site mirror above turned into queryable data - four flat JSON arrays, extracted locally from it. `conversations.json`: 48 records, the 12 scripted conversations as **231 aligned dialogue lines** plus the 36 audio-only "real conversations" as summaries. **Every dialogue line carries five parallel tracks** - Kansai-ben, standard Japanese, English, grammar annotations and word annotations - each annotation resolved inline to the grammar point or word it names (544 of 550 grammar references, all 315 word references). Then `words.json`, 280 dialect word entries with standard equivalents, register warnings and examples; `grammar.json`, 61 records (45 grammar points + 16 phonology topics) as structured blocks and tables; `glossary.json`, 62 jargon terms. **CC BY-SA 3.0, and share-alike is inherited** - whatever you build from it carries the same licence.   | **The only aligned Kansai/standard parallel corpus and the only dialect vocabulary dataset in the repo**, and the form the [recognition kit](#kansai-ben-recognition-kit) above wants: every `へん`, `や`, `あかん` and `ねん` sits on a line whose standard-Japanese equivalent is one field away, so you can build drill pairs or mine dialect cards without parsing HTML. Its own `README.md` lists every field. **Honest limits:** 6 grammar cross-references dangle, their pages being absent from the mirror; 28 grammar tables have label-only rows where an unmirrored audio clip sat; and two source-side defects - a missing comma in one Kansai line, a stray paren in another - are reproduced verbatim, not repaired.    |
