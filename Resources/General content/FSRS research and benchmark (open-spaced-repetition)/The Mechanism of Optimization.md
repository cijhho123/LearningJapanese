...
FSRS is based on the **DSR** (Difficulty, Stability, Retrievability) memory model. The **DSR** model uses seventeen parameters and six equations to describe memory dynamics during spaced repetition practice (For details, see [The Algorithm](https://github.com/open-spaced-repetition/awesome-fsrs/wiki/The-Algorithm)).

Here is a brief introduction to the training process of FSRS.

# Pre-processing review logs

The database schema of review logs is as follows (Copy from [Database Structure](https://github.com/ankidroid/Anki-Android/wiki/Database-Structure)):
```SQL
