...
| Itazuraneko - extracted data sets | [Extracted Data (JSON-CSV)/](<./Itazuraneko Master Reference/Extracted Data (JSON-CSV)>), [Novel Library Kanji Inventories (JSON)/](<./Itazuraneko Master Reference/Novel Library Kanji Inventories (JSON)>) | JSON + CSV, 39.5 MB | The 文型一覧表, DoJG, どんなときどう使う, Kanken kanji and radical tables above, pulled out of the HTML as data - plus per-volume kanji inventories for the mirror's novel library. Listed in full under [machine-readable grammar data](#machine-readable-grammar-data). |
...
Everything above is prose. Everything below is data - greppable, joinable, scriptable. The tables in this guide are hand-written and finite (a 20-row conjugation map, a 30-pair transitivity table, a 13-verb keigo table); these files are the same information at 10-100x the coverage. Licensing, attribution requirements, verified entry counts and a "declined and why" list are in [sources.md](./sources.md). The last two sets were **extracted locally from the Itazuraneko mirror already in this folder** rather than downloaded from anywhere; everything else came from upstream.
...
| **Itazuraneko - reference tables as data** | [masterreference.json](<./Itazuraneko Master Reference/Extracted Data (JSON-CSV)/masterreference.json>), [dojg.json](<./Itazuraneko Master Reference/Extracted Data (JSON-CSV)/dojg.json>), [donnatoki.json](<./Itazuraneko Master Reference/Extracted Data (JSON-CSV)/donnatoki.json>), [kanji-ichiranhyou](<./Itazuraneko Master Reference/Extracted Data (JSON-CSV)/kanji-ichiranhyou.json>), [radicals](<./Itazuraneko Master Reference/Extracted Data (JSON-CSV)/radicals.json>) | 5 JSON + 3 CSV, 6.0 MB, **extracted locally** from the mirror listed under [references](#references-and-dictionaries) rather than downloaded. Fields are documented in the folder's own [README](<./Itazuraneko Master Reference/Extracted Data (JSON-CSV)/README.md>) | The mirror's reference tables, queryable. **`masterreference.json` is the one to reach for: 2,628 grammar points, each carrying the book that covers it** plus a key into `dojg.json` (**629** entries), `donnatoki.json` (**631** real entries + **401** redirect stubs) or an HJG entry number (968) - so "what is this pattern called, and where is it explained" becomes one join instead of five lookups across the HTML above. `dojg.json` adds formation grids, key sentences, examples, character-offset highlight spans and typed cross-references. **Caveat: DoJG's Notes field is a scanned image on 416 of the 629 entries and no OCR was attempted** - all 228 advanced and 188 of 198 intermediate entries carry an image path rather than note text; every basic entry has text. `kanji-ichiranhyou` (**27,506** kanji by Kanken level) and `radicals` (**321**) are in the same folder and are **not grammar** - they are kept with the mirror they came out of so the provenance stays intact, and [../Kanji/readme.md](../Kanji/readme.md) points at them from there |
| **Itazuraneko - novel library kanji inventories** | [volumes.json](<./Itazuraneko Master Reference/Novel Library Kanji Inventories (JSON)/volumes.json>), [titles.json](<./Itazuraneko Master Reference/Novel Library Kanji Inventories (JSON)/titles.json>) | 2 JSON, **8,160** volumes over **5,456** works, 33.5 MB, extracted locally. [README](<./Itazuraneko Master Reference/Novel Library Kanji Inventories (JSON)/README.md>) | **The "can I read this yet" file, and nothing else in this repo does it**: exactly which kanji appear in each volume of the Itazuraneko novel library, joined to title, author and volume order. Diff a volume against your known-kanji list and you have the gap for that specific book before you start it - which is what justifies the 33.5 MB. Two facts before you load it: the kanji are **one concatenated string per volume, not an array** (that choice is why the file is 33 MB rather than 60), and they are in **first-appearance order, not frequency order - do not re-sort**. **Caveat: 313 volumes hold under 50 kanji. Those are source-side stubs, not parse failures** - Itazuraneko's own extractor only saw a cover or contents page. The library being catalogued is piracy-adjacent; what is reproduced here is bibliographic metadata and character inventories, not book text. Really a **reading** resource, filed here because the mirror is - see [../Reading/readme.md](../Reading/readme.md) |
